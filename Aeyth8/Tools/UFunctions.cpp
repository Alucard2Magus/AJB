...
#include "../../Dumper-7/SDK/BP_AJBC29Melee_classes.hpp"
#include "../../Dumper-7/SDK/BP_AJBC30Melee_classes.hpp"
...
void LemonPossession()
{
	...
	if (LemonEssence)
	{
		...
		for (int i{0}; i < SDK::UObject::GObjects->Num(); ++i)
		{
			...
			{				
				static uint64 ImageSetBrushFromMaterial(PB(0x10C7F20));
			...
			{
				static uint64 BorderSetBrushFromMaterial(PB(0x10C7E20));
...
SDK::FString* UFunctions::ConsoleCommand(SDK::APlayerController* This, SDK::FString* Result, SDK::FString* Command, bool bWriteToLog)
{
	...
	else if (StrCommand.find("AJBExecInternalMode") == 0)
	{
		...
		case 2:  // Pair
		case 3:  // Shop
		case 4:  // ShopCompetition
		case 7:  // ShopCompetitionPair
		case 8:  // ShopExchange
		case 10: // PVEPair
			AJB::Instance->bIsLocalSessionMode = true;
			break;
	...
	// --- Pair Mode Commands ---
	else if (StrCommand == "team")
	{
		// Show current team and partner info in console
		std::wstring msg = L"[PairMode] Team: '";
		if (PairMode::MyTeamName.empty())
			msg += L"(none/solo)";
		else
			msg += std::wstring(PairMode::MyTeamName.begin(), PairMode::MyTeamName.end());
		msg += L"'";

		// Find partner from LockedTeamIDs
		if (AJB::Instance && AJB::Instance->MatchingPlayers.Num() > 0)
		{
			// Find my key
			std::string myKey;
			if (AJB::IsServer())
			{
				myKey = AJB::Instance->MatchingPlayers[0].First.ToString();
			}
			else
			{
				// Client: find by CharactorID
				uint8 myChar = static_cast<uint8>(AJB::TEMP_CachedCharacterID);
				for (int i = 0; i < AJB::Instance->MatchingPlayers.Num(); ++i)
				{
					if (AJB::Instance->MatchingPlayers[i].Second.CharactorID == myChar)
					{
						myKey = AJB::Instance->MatchingPlayers[i].First.ToString();
						break;
					}
				}
			}

			// Find partner — try LockedTeamIDs (host), then scan TMap (client)
			bool foundPartner = false;

			// Host path: use LockedTeamIDs
			if (PairMode::LockedTeamIDs.count(myKey))
			{
				int32 myTeamID = PairMode::LockedTeamIDs[myKey];
				for (auto& [key, tid] : PairMode::LockedTeamIDs)
				{
					if (tid == myTeamID && key != myKey)
					{
						msg += L" | Partner: ";
						msg += std::wstring(key.begin(), key.end());
						msg += L" (TeamID " + std::to_wstring(myTeamID) + L")";
						foundPartner = true;
						break;
					}
				}
			...
			// Client fallback: scan TMap for matching TeamID
			if (!foundPartner && !myKey.empty())
			{
				int32 myTeamID = 0;
				int myIdx = -1;
				for (int i = 0; i < AJB::Instance->MatchingPlayers.Num(); ++i)
				{
					std::string k = AJB::Instance->MatchingPlayers[i].First.ToString();
					if (k == myKey || k.find(myKey) == 0 || myKey.find(k) == 0)
					{
						myTeamID = AJB::Instance->MatchingPlayers[i].Second.TeamID;
						myIdx = i;
						break;
					}
				}
				if (myTeamID > 0 && myIdx >= 0)
				{
					for (int i = 0; i < AJB::Instance->MatchingPlayers.Num(); ++i)
					{
						if (i == myIdx) continue;
						if (AJB::Instance->MatchingPlayers[i].Second.TeamID == myTeamID)
						{
							std::string partKey = AJB::Instance->MatchingPlayers[i].First.ToString();
							msg += L" | Partner: ";
							msg += std::wstring(partKey.begin(), partKey.end());
							msg += L" (TeamID " + std::to_wstring(myTeamID) + L")";
							foundPartner = true;
							break;
						}
			...
			if (!foundPartner)
			{
				msg += L" | Waiting for partner...";
			}
		}

		ConsoleOutput::Text(msg);
		PairMode::HandleCommand(StrCommand);
	}
	else if (PairMode::HandleCommand(StrCommand))
	{
		// Handled by PairMode
	}
	// --- End Pair Mode Commands ---
...
UFunctions::BrowseReturnVal UFunctions::Browse(SDK::UEngine* This, SDK::FWorldContext& WorldContext, SDK::FURL URL, SDK::FString& Error)
{
	...
	if (URL.Map.CStr()) LogA("Browse", Helpers::FURLParser(URL));
...
void UFunctions::NotifyControlMessage(SDK::UPendingNetGame* This, SDK::UNetConnection* Connection, uint8 MessageType, void* InBunch)
{
	...
	LogA(OFF::NotifyControlMessage.GetName(), std::format("[UPendingNetGame]: 0x{:X} | [Connection]: 0x{:X} | [MessageType]: {}", (uint64)This, (uint64)Connection, MessageType));
...
void UFunctions::InitLocalConnection(SDK::UNetConnection* This, SDK::UNetDriver* InDriver, void* InSocket, SDK::FURL& InURL, EConnectionState InState, int InMaxPacket, int InPacketOverhead)
{
	...
	// Pair Mode: append ?PairTeam=xxx to the URL options on the client side
	PairMode::OnInitLocalConnection(InURL);
...
void AJBPreLogin(SDK::AGameModeBase* This, SDK::FString* Options, SDK::FString* Address, SDK::FUniqueNetIdRepl* UniqueId, SDK::FString* ErrorMessage)
{
	...
	// Pair Mode: extract ?PairTeam=xxx from login Options on the host
	if (Options) PairMode::OnPreLogin(Options->ToString());
...
void UFunctions::PreLogin(SDK::AGameModeBase* This, SDK::FString* Options, SDK::FString* Address, SDK::FUniqueNetIdRepl* UniqueId, SDK::FString* ErrorMessage)
{
	...
	// Pair Mode: extract ?PairTeam=xxx from login Options on the host (lobby map)
	if (Options) PairMode::OnPreLogin(Options->ToString());
}
...
void UFunctions::PostLogin(SDK::AGameModeBase* This, SDK::APlayerController* Player)
{
	...
	if (AJB::IsServer())
	{
		if (!AJB::MOD_Global_Synchronizer) AJB::MOD_Global_Synchronizer = Pointers::SpawnActor<SDK::ABP_Synchronizer_C>(Pointers::FActorSpawnParameters{static_cast<unsigned char>(SDK::ESpawnActorCollisionHandlingMethod::AlwaysSpawn)});
	...
	// Pair Mode: detect guest joining and set up pair data
	PairMode::OnPostLogin(This, Player);
...
__int64* UFunctions::SpawnActor(SDK::UWorld* This, SDK::UClass* Class, const SDK::FVector* Location, const SDK::FRotator* Rotation, FActorSpawnParameters& SpawnParameters)
{
	...
	std::string SpawnParms = "[Name]: " + SpawnParameters.Name.ToString()
		+ " | [Template]: " + (SpawnParameters.Template ? SpawnParameters.Template->GetFullName() : "NULL")
		+ " | [Owner]: " + (SpawnParameters.Owner ? SpawnParameters.Owner->GetFullName() : "NULL")
		+ " | [Instigator]: " + (SpawnParameters.Instigator ? SpawnParameters.Instigator->GetFullName() : "NULL")
		+ " | [OverrideLevel]: " + (SpawnParameters.OverrideLevel ? SpawnParameters.OverrideLevel->GetFullName() : "NULL")
		+ " | [SpawnCollisionHandlingOverride]: " + SDT_SpawnCollision[static_cast<int>(SpawnParameters.SpawnCollisionHandlingOverride)];
...
{
	// === PAIR CAMERA DIAG ===
	// Log Invoke during pair-mode InGame.Gameplay only. Skips per-frame noise.
	// Goal: capture the UFunction names fired by LMB/RMB clicks so we can find
	// the function that flips camera into RMB-drag mode and block it.
	if (PairMode::bIsPairSession && AJB::CurrentFlowstate)
	{
		static SDK::FName Gameplay = FName::NAME_FindOrAdd(L"InGame.Gameplay");
		if (AJB::CurrentFlowstate->TagName == Gameplay)
		{
			const std::string FnName = This->GetName();
			if (FnName.find("Tick") == std::string::npos
				&& FnName.find("Update") == std::string::npos
				&& FnName.find("ReceiveTick") == std::string::npos
				&& FnName.find("BlueprintUpdateAnimation") == std::string::npos
				&& FnName.find("BlueprintPostEvaluateAnimation") == std::string::npos)
			{
				LogA("PairDiag", std::format("[Fn]: {} | [Obj]: {}", FnName, Obj->GetName()));
			}
		}
	}
